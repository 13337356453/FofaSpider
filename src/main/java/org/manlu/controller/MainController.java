...
public class MainController implements Initializable {
    ...
    @FXML
    private Button ico_cal_btn;

    @FXML
    private Button ico_choice_path_btn;

    @FXML
    private Button ico_copy_btn;

    @FXML
    private TabPane tabpane;

    @FXML
    private TextField ico_file_path;

    @FXML
    private ImageView ico_img;

    @FXML
    private TextArea ico_result;

    @FXML
    private Button ico_send_btn;
...
//            api_result.getItems().clear();
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        loadWebView();
        // 初始化imageview
        ico_img.setFitWidth(370);
        ico_img.setPreserveRatio(true);
    ...
    private void loadWebView(){
        ...
        webEngine.load(getClass().getResource("/org/manlu/html/hash.html").toExternalForm());
    }
    ...
    @FXML
    void ico_cal(ActionEvent event) throws IOException {
        String path=ico_file_path.getText().strip();
        if(!path.strip().equals("") && ico_img.getImage()!=null){
            File file=new File(path);
            byte[] fileContent= Files.readAllBytes(file.toPath());
            String content=Base64.getEncoder().encodeToString(fileContent);
            Object hash=webEngine.executeScript("calculate('"+content+"')");
            ico_result.setText(hash.toString());
        }else {
            Alerter.ErrorAlert("错误",null,"请选择图片文件",MainFrame.stage);
        }
    }

    @FXML
    void ico_choice_path(ActionEvent event) {
        FileChooser fileChooser = new FileChooser();
        fileChooser.setTitle("选择文件");
        fileChooser.getExtensionFilters().addAll(new FileChooser.ExtensionFilter("图片文件","*.ico;*.jpg;*.png;*.gif"),new FileChooser.ExtensionFilter("全部文件","*.*"));
        File file=fileChooser.showOpenDialog(MainFrame.stage);
        if (file != null) {
            ico_file_path.setText(file.toString());
            Image image=new Image(file.toURI().toString());
            ico_img.setImage(image);
        }
    }

    @FXML
    void ico_copy(ActionEvent event) {
        String hash=ico_result.getText().strip();
        if (!hash.equals("")) {
            Clipboard clipboard = Toolkit.getDefaultToolkit().getSystemClipboard();
            Transferable trans = new StringSelection(hash);
            clipboard.setContents(trans, null);
        }
    }

    @FXML
    void ico_send(ActionEvent event) {
        String hash=ico_result.getText().strip();
        if (!hash.equals("")) {
            kw_icon_field.setText(hash);
            tabpane.getSelectionModel().select(kw_tab);
        }
    }
}
